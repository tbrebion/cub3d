...
/*   Updated: 2022/09/12 14:49:09 by tbrebion         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ray_ver(void)
{
	...
	// {
		...
		y = g_data.pos.y + (x - g_data.pos.x) * (g_data.ray.y/*raydir_y*/ / /*raydir_x*/g_data.ray.x);
		...
		g_data.hit.y = g_data.pos.y;		// side = which_side();
...
void	ray_rotate(void)
{
	...
	angle = ((double)g_data.ray.i - (g_data.win.x / 2)) * 33 / (g_data.win.x / 2);
...
{
	while (g_data.ray.i < /*g_data.win.x*/W)
	{
		ray_rotate();
		ft_dir();
		ray_ver();
